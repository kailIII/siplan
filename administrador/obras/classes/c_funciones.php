...
function sac_comp($ido){
$consulta_nop = mysql_query("SELECT * FROM obras where id_obra=".$ido ) or die (mysql_error());
$res_nop = mysql_fetch_array($consulta_nop);

$consulta_nop = mysql_query("SELECT * FROM componentes where id_componente=".$res_nop['id_componente'] ) or die (mysql_error());
$res_nop = mysql_fetch_array($consulta_nop);


return $res_nop['no_componente'];
}
