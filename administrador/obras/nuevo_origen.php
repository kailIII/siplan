...
elseif($val1==3){
echo '<script type="text/javascript">';
echo 'alert ("No es posible asignar la obra, verifique que corresponda a la partida, origen, proyecto, componente y accion");';
echo 'window.location="main.php?token=e369853df766fa44e1ed0ff613f563bd&d='.$val2.'&p='.$val3.'&f_p='.$ff_p.'"; ';
echo '</script>';
}
...
elseif($val1==7){
echo '<script type="text/javascript">';
echo "alert('Para realizar la cancelacion es necesario tener un monto previamente asignado de la misma partida, origen, proyecto, componente y accion.');";
echo 'window.location="main.php?token=e369853df766fa44e1ed0ff613f563bd&d='.$val2.'&p='.$val3.'&f_p='.$ff_p.'"; ';
echo '</script>';
}
...
elseif($val1==8){
echo '<script type="text/javascript">';
echo "alert('El monto de la cancelacion excede el monto acumulado.');";
echo 'window.location="main.php?token=e369853df766fa44e1ed0ff613f563bd&d='.$val2.'&p='.$val3.'&f_p='.$ff_p.'"; ';
echo '</script>';
}
 ...
 var er_dom = /^([0-9]|[a-z]|[$,."';:()]|#|[A-Z]|á|é|í|ó|ú|ñ|Ñ|ü|Á|É|Í|Ó|Ú|Ü|\s|\.|-)+$/
    ...
    $(".btn_medida").click(function (){
        $(".error").remove();
		 ...
		 if( $("#proycompacc").val() == "" ){
            $("#proycompacc").focus().after("<span class='error'>Seleccione Proy. / Comp. / Act.</span>");
            return false;
        }
...
<p><h3>Agregar Recursos</h3>
  ...
  <select name="proycompacc" id="proycompacc" onclick="mostrar_partida(this.value)">
  	...
  	<?php
	  $consulta_num_pca = mysql_query("SELECT * FROM poa02_origen WHERE id_obra = ".$id_obra." and id_proyecto=".$_GET['p']." and tipo=0 GROUP BY s11c_compon, s25c_accion") or die (mysql_error());
	  $nnu=mysql_num_rows($consulta_num_pca);
	  if ($nnu>=1){

	  }
	  else{

		$consulta_num_pca = mysql_query("SELECT * FROM estados_financieros where s02c_depend = ".$_SESSION['id_dependencia_v3'] ." AND s06c_proyec = ".$f->sac_noproyecto($_GET['p'])." and  s11c_compon=".$f->sac_comp($_GET['d'])." GROUP BY s11c_compon, s25c_accion",$siplan_data_conn) or die (mysql_error());
	  }
...
<select name="origen" id="origen">
	...
	<?php
	  $consultar_poa = mysql_query("SELECT sum(monto) as total FROM poa02_origen WHERE id_obra = ".$id_obra." and id_proyecto=".$_GET['p']."  and tipo=0  and status_of>=1 and id_poa02!=''");
	  $det_val=mysql_fetch_array( $consultar_poa);
	  $tot=$det_val['total'];

	 if ($tot>0) {?>
  <p>Tipo de Movimiento
		<select name="tpmv" id="tpmv">
       <option value="" selected="selected">-seleccione-</option>
 ...
 <?php  }?>
  ...
  <input type="text" name="monto" id="monto" value="0"  class="right currency">
   ...
   <p><b>Monto Asignado a la obra:</b>
  	<?php
  	   $consultar_montos = mysql_query("SELECT * FROM poa02_origen WHERE id_obra = ".$id_obra." and id_proyecto=".$_GET['p']." and tipo IN (0,1)") or die (mysql_error());
	   $monto = 0.00;

	 while($res_montos = mysql_fetch_array($consultar_montos)){
	   	   $monto = $monto + $res_montos['monto'];
	   	}
		 $consultar_montos_can = mysql_query("SELECT * FROM poa02_origen WHERE id_obra = ".$id_obra." and id_proyecto=".$_GET['p']." and tipo=2") or die (mysql_error());
	   $monto_can = 0.00;

	 while($res_montos_can = mysql_fetch_array($consultar_montos_can)){
	   	   $monto_can = $monto_can + $res_montos_can['monto'];
	   	}


	   echo "$ ".number_format($monto-$monto_can,2);
...
<table width="100%" cellpadding="0" cellspacing="0" border="0" class="display" id="example3">
 <thead>
    <tr>
	 ...
	 <td width="7%"><div align="center"><b>Componente</b></div></td>
  	<td width="7%"><div align="center"><b>Actividad</b></div></td>
	<td width="7%"><div align="center"><b>Monto</b></div></td>
      <td width="7%"><div align="center"><b>Tipo</b></div></td>
 ...
 <tr class="<?php echo $css_color; ?>">
   <td ><?php echo $resobras['id_poa02']; ?></td>
	...
	?>
        <td align="middle"><?php echo $tp;?></td>

   <td align="middle"><div align="center">

    <?php if ($resobras['status_of']!=2){ ?>
    ...
    <?php } else{ ?>
    <img src="imagenes/delete_dis_2.png" width="24" height="24" title="Eliminar">
    <?php  }?>
    </div></td>
 </tr>
    <?php } ?>
	...
	<td width="7%"><div align="center">Monto</div></td>
      <td width="7%"><div align="center">Tipo</div></td>
