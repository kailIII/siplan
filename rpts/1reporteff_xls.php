...
header("Content-Disposition: attachment; filename=AFF_POA_todas_obras_2014.xls");
...
<table border=0 cellpadding=0 cellspacing=0 width=3422 style='border-collapse:
 ...
 <tr height=30 style='mso-height-source:userset;height:22.5pt'>
  ...
  height:22.5pt'>Avance de Ejercicio Físico - Financiero <?php echo date("Y"); ?></td>
 </tr>
...
$query.=" and dependencias.id_dependencia=".$dep;
	 ...
	 // $consulta_aas = mysql_query("SELECT *,(obras.cantidad) as cant_pro FROM oficio_aprobacion INNER JOIN ((sectores INNER JOIN (regiones INNER JOIN (municipios INNER JOIN ((dependencias INNER JOIN (acuerdos INNER JOIN proyectos ON acuerdos.id_acuerdo = proyectos.id_acuerdo) ON dependencias.id_dependencia = proyectos.id_dependencia) INNER JOIN obras ON (proyectos.id_proyecto = obras.id_proyecto) AND (dependencias.id_dependencia = obras.id_dependencia)) ON municipios.id_municipio = obras.municipio) ON regiones.id_region = municipios.id_region) ON sectores.id_sector = dependencias.id_sector) INNER JOIN detalle_oficio ON (proyectos.id_proyecto = detalle_oficio.id_proyecto) AND (obras.id_obra = detalle_oficio.id_poa02)) ON oficio_aprobacion.id_oficio = detalle_oficio.id_oficio WHERE obras.status_obra BETWEEN 2 AND 3 AND oficio_aprobacion.tipo=0 AND oficio_aprobacion.no_oficio !='' ".$query." GROUP BY detalle_oficio.id_detalle_of ORDER BY dependencias.id_dependencia, proyectos.no_proyecto, obras.obra",$siplan_data_conn) or die (mysql_error());


	   $consulta_obras = mysql_query("SELECT *,(obras.cantidad) as cant_pro FROM sectores INNER JOIN (regiones INNER JOIN (municipios INNER JOIN ((dependencias INNER JOIN obras ON dependencias.id_dependencia = obras.id_dependencia) INNER JOIN (eje INNER JOIN proyectos ON eje.id_eje = proyectos.id_eje) ON (proyectos.id_proyecto = obras.id_proyecto) AND (dependencias.id_dependencia = proyectos.id_dependencia)) ON municipios.id_municipio = obras.municipio) ON regiones.id_region = municipios.id_region) ON sectores.id_sector = dependencias.id_sector WHERE obras.status_obra BETWEEN 3 AND 4 ".$query." GROUP BY obras.id_obra ORDER BY dependencias.id_dependencia, proyectos.no_proyecto, obras.obra",$siplan_data_conn) or die (mysql_error());
  ...
  $sector=(utf8_decode($res_sec['sector']));
  //termina sector
 ...
 //regiones
  ...
  $region=('0'.$res_reg['id_region'].', '.utf8_decode($res_reg['nombre']));
  }else{
   $region=($res_reg['id_region'].', '.utf8_decode($res_reg['nombre']));
  }
  //termina regiones
 ...
 $sac_poa = mysql_query("SELECT * FROM programas_poa WHERE id_programa_poa = ".$row['programa_poa'],$siplan_data_conn) or die (mysql_error());
  ...
  $poa=($res_poa['clave'].', '.utf8_decode($res_poa['descripcion']));
 //terminna poa
 ...
 $sac_subpoa = mysql_query("SELECT * FROM subprogramas_poa WHERE id_subprograma_poa = ".$row['subprograma_poa'],$siplan_data_conn) or die (mysql_error());
  ...
  $subpoa=($res_subpoa['clave'].', '.utf8_decode($res_subpoa['descripcion']));
 //subterminna poa

 //descripcion obra
  $nom_obras=(utf8_decode($row['descripcion']));
 ...
 // termina clave municipio
  ...
  //nombre municipio
 $nom_mun=(utf8_decode($res_mun['nombre']));
...
// SELECT * FROM localidades WHERE id_localidad = ".$row['localidad']
  ...
  // termina clave localidad
   ...
   //nombre localidad
 $nom_loc=(utf8_decode($res_loc['nombre']));
...
$otr=number_format($res_ejercido5['otr'],2);
 ...
 //unida de medidad
  $sac_medida = mysql_query("select  * from unidad_medida_prog02 where  id_unidad = ".$row['u_medida'],$siplan_data_conn) or die (mysql_error());
	...
	$medida=(utf8_decode($res_medida['descripcion']));
 //termian medidad
...
$st_edo="Cancelada";
 ...
 <tr class=xl144 height=17 style='height:12.75pt'>
  <td height=17 class=xl115 width=3422 style='height:12.75pt;width:53pt'><?php echo ((($sector))); ?></td>
