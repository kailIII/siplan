 ...
 */
 ...
 $query="SELECT dependencias.acronimo, dependencias.id_dependencia, Sum(aportes.federal) AS SumaDefederal, Sum(aportes.estatal) AS SumaDeestatal, Sum(aportes.municipal) AS SumaDemunicipal, Sum(aportes.participantes) AS SumaDeparticipantes, Sum(aportes.credito) AS SumaDecredito, Sum(aportes.otros) AS SumaDeotros, Sum(aportes.federal+aportes.estatal+aportes.municipal+aportes.participantes+aportes.credito+aportes.otros) AS total, Count(dependencias.acronimo) AS CuentaDeacronimo
...
elseif($mp=="59") {
	$query="SELECT regiones.id_region, dependencias.id_dependencia, (regiones.nombre) as nomreg, dependencias.acronimo, (dependencias.id_dependencia) as nd,  Sum(aportes.federal+aportes.estatal+aportes.municipal+aportes.participantes+aportes.credito+aportes.otros) AS total
...
/*switch($_SESSION["id_perfil_v3"]){
	case 1:
	echo "<link href='../estilo/stylo.css' rel='stylesheet' type='text/css' media='all'>";
	break;
...
<a href="#"  style="text-decoration:none; color:#000"	onclick="$('#datoss_<?php echo $resobras['id_dependencia'];?>').load('obras.php?dep=<?php echo $resobras['id_dependencia']."&mp=".$_GET['mp'];?>');"  style="cursor: pointer;"  ><b>
...
<div id="datoss<?php echo "_".$resobras['id_dependencia']; ?>">
</div>

<a class="print"  style="  text-decoration:none; color:white;

font-size: 1.em" target="_blank"  href="reporte_obras_gral_res.php?dep=<?php echo $resobras['id_dependencia']."&mp=".$_GET['mp'];?>">Imprimir</a><a class="cerrar">Cerrar</a>
</div>
</div>
...
elseif($mp=="59") {
	...
	FROM `siplan`.`municipios` AS `municipios`, `siplan`.`regiones` AS `regiones`, `siplan`.`obras` AS `obras`, `siplan`.`dependencias` AS `dependencias`, `siplan`.`poa02_origen` AS `poa02_origen`
	 WHERE `municipios`.`id_region` = `regiones`.`id_region` AND `obras`.`municipio` = `municipios`.`id_finanzas` AND `obras`.`id_dependencia` = `dependencias`.`id_dependencia` AND `obras`.`id_obra` = `poa02_origen`.`id_obra`
	  AND `poa02_origen`.`id_oficio` != 0 AND `poa02_origen`.`status_of` = 2
	AND `poa02_origen`.`s07c_partid` BETWEEN 4000 AND 4999
	OR `municipios`.`id_region` = `regiones`.`id_region` AND `obras`.`municipio` = `municipios`.`id_finanzas` AND `obras`.`id_dependencia` = `dependencias`.`id_dependencia` AND `obras`.`id_obra` = `poa02_origen`.`id_obra`
	  AND `poa02_origen`.`id_oficio` != 0 AND `poa02_origen`.`status_of` = 2
	 AND `poa02_origen`.`s07c_partid` BETWEEN 6000 AND 6999
	   GROUP BY `regiones`.`id_region`";


	/*SELECT regiones.id_region, (regiones.nombre) as nomreg, dependencias.acronimo, (dependencias.id_dependencia) as nd,  Sum(aportes.federal+aportes.estatal+aportes.municipal+aportes.participantes+aportes.credito+aportes.otros) AS total
FROM regiones INNER JOIN (municipios INNER JOIN (dependencias INNER JOIN (obras INNER JOIN aportes ON obras.id_obra = aportes.id_obra) ON dependencias.id_dependencia = obras.id_dependencia) ON municipios.id_finanzas = obras.municipio) ON regiones.id_region = municipios.id_region where obras.status_obra=4
 GROUP BY regiones.id_region";*/
...
<a href="#"  style="text-decoration:none; color:#000"	onclick="$('#datoss_ap<?php echo $resobras['id_dependencia'];?>').load('obras.php?ap=1&dep=<?php echo $resobras['id_dependencia']."&mp=".$_GET['mp'];?>');"  style="cursor: pointer;"  ><b>
	<?php
...
<div id="datoss<?php echo "_ap".$resobras['id_dependencia']; ?>">
</div>

<a class="print"  style="  text-decoration:none; color:white;

font-size: 1.em" target="_blank"  href="reporte_obras_gral_res.php?dep=<?php echo $resobras['id_dependencia']."&ap=1&mp=".$_GET['mp'];?>">Imprimir</a><a class="cerrar">Cerrar</a>
</div>
</div>
...
WHERE municipios.id_finanzas =".$mp." group by localidades.id_marginacion";*/
		 ...
		 FROM `siplan`.`localidades` AS `localidades`, `siplan`.`obras` AS `obras`, `siplan`.`municipios` AS `municipios`, `siplan`.`poa02_origen` AS `poa02_origen`
		 WHERE `localidades`.`id_finanzas` = `obras`.`localidad` AND `obras`.`municipio` = `municipios`.`id_finanzas` AND `municipios`.`id_municipio` = `localidades`.`id_municipio` AND `obras`.`id_obra` = `poa02_origen`.`id_obra`
		 ...
		 AND `poa02_origen`.`s07c_partid` BETWEEN 4000 AND 4999
OR  `localidades`.`id_finanzas` = `obras`.`localidad` AND `obras`.`municipio` = `municipios`.`id_finanzas` AND `municipios`.`id_municipio` = `localidades`.`id_municipio` AND `obras`.`id_obra` = `poa02_origen`.`id_obra`
		 AND `obras`.`municipio` = ".$mp." AND `obras`.`status_obra` = '4'
		 AND `poa02_origen`.`tipo` = 0 AND `poa02_origen`.`id_oficio` != 0 AND `poa02_origen`.`status_of` = 2
		 AND `poa02_origen`.`s07c_partid` BETWEEN 6000 AND 6999
...
//echo $nam1, (number_format((($rowm['total']+$rowres1['total'])-$rowres2['total']),2,".",""));

	$dataSet->addPoint(new Point( $nam1, (number_format((($rowm['total']+$rowres1['total'])-$rowres2['total']),2,".",""))));
 //(number_format($rowm['total'],2,".",""))));
...
<?php
  ...
  $quer1="
SELECT `localidades`.`id_marginacion`, `poa02_origen`.`id_oficio`, `poa02_origen`.`status_of`, SUM( `poa02_origen`.`monto` ) AS `total` FROM `siplan`.`localidades` AS `localidades`, `siplan`.`obras` AS `obras`, `siplan`.`municipios` AS `municipios`, `siplan`.`regiones` AS `regiones`, `siplan`.`poa02_origen` AS `poa02_origen`
 WHERE `localidades`.`id_finanzas` = `obras`.`localidad` AND `municipios`.`id_municipio` = `localidades`.`id_municipio` AND `municipios`.`id_finanzas` = `obras`.`municipio` AND `regiones`.`id_region` = `municipios`.`id_region` AND `poa02_origen`.`id_obra` = `obras`.`id_obra`
 ...
 GROUP BY `localidades`.`id_marginacion`";
  ...
  $quer2="SELECT `localidades`.`id_marginacion`, `poa02_origen`.`id_oficio`, `poa02_origen`.`status_of`, SUM( `poa02_origen`.`monto` ) AS `total` FROM `siplan`.`localidades` AS `localidades`, `siplan`.`obras` AS `obras`, `siplan`.`municipios` AS `municipios`, `siplan`.`regiones` AS `regiones`, `siplan`.`poa02_origen` AS `poa02_origen`
 WHERE `localidades`.`id_finanzas` = `obras`.`localidad` AND `municipios`.`id_municipio` = `localidades`.`id_municipio` AND `municipios`.`id_finanzas` = `obras`.`municipio` AND `regiones`.`id_region` = `municipios`.`id_region` AND `poa02_origen`.`id_obra` = `obras`.`id_obra`
 AND `regiones`.`id_region` = ".$rowm['id_region']." and localidades.id_marginacion=".$rowm['id_marginacion']." AND `obras`.`status_obra` = '4' AND `poa02_origen`.`id_oficio` != 0 AND `poa02_origen`.`status_of` = 2
...
echo '<img alt="Line chart"  src="tmp/ma'.$rowrr['id_region'].'_ap.png" style="border: 1px solid gray;"/>';
 ...
 /*anterior
    $quer="SELECT localidades.id_marginacion, dependencias.acronimo, (dependencias.id_dependencia) AS nd, Sum(aportes.federal+aportes.estatal+aportes.municipal+aportes.participantes+aportes.credito+aportes.otros) AS total FROM (regiones INNER JOIN (dependencias INNER JOIN (municipios INNER JOIN (obras INNER JOIN aportes ON obras.id_obra = aportes.id_obra) ON municipios.id_finanzas = obras.municipio) ON dependencias.id_dependencia = obras.id_dependencia) ON regiones.id_region = municipios.id_region) INNER JOIN localidades ON (localidades.id_finanzas = obras.localidad) AND (municipios.id_municipio = localidades.id_municipio) WHERE obras.municipio=".$mp." and obras.status_obra>=3 GROUP BY  dependencias.id_dependencia, localidades.id_marginacion";*/
	    ...
	    /*SELECT localidades.id_marginacion, dependencias.acronimo, (dependencias.id_dependencia) AS nd, Sum(aportes.federal+aportes.estatal+aportes.municipal+aportes.participantes+aportes.credito+aportes.otros) AS total FROM (dependencias INNER JOIN (municipios INNER JOIN (obras INNER JOIN aportes ON obras.id_obra = aportes.id_obra) ON municipios.id_finanzas = obras.municipio) ON dependencias.id_dependencia = obras.id_dependencia) INNER JOIN localidades ON (municipios.id_municipio = localidades.id_municipio) AND (obras.localidad = localidades.id_finanzas) WHERE obras.municipio=".$mp." and obras.status_obra=4 GROUP BY  dependencias.id_dependencia, localidades.id_marginacion";
...
$rowres2=mysql_fetch_array($consulta_obrasmm2);
 ...
 <tr   style="cursor: pointer;" >
   <td align="justify"><?php echo $resobras['acronimo']; ?></td>
	...
	// echo $resobras['id_marginacion']." ".$resobras['acronimo']." ".$resobras['total']." ".$rowres1['acronimo']." ".$rowres1['id_marginacion']." ".$rowres2['acronimo']." ".$rowres2['id_marginacion']."--> ".$resobras['total']." ".$rowres1['total']." ".$rowres2['total'];
