...
header("Content-Disposition: attachment; filename=AFF01_POA_2014.xls");
...
// SELECT * FROM localidades WHERE id_localidad = ".$row['localidad']
 ...
 $sumar_poa02 = mysql_query("select  (monto) as total  from poa02_origen where id_poa02 = ".$row['obra']." and tipo=0",$siplan_data_conn) or die (mysql_error());
	$sum_aporte= mysql_fetch_array($sumar_poa02);

	 $sumar_add = mysql_query("select  (monto) as total  from poa02_origen where id_poa02 = ".$row['obra']." and tipo=1",$siplan_data_conn) or die (mysql_error());
	$sum_aporte_add= mysql_fetch_array($sumar_add);

	 $sumar_rest = mysql_query("select  (monto) as total  from poa02_origen where id_poa02 = ".$row['obra']." and tipo=2",$siplan_data_conn) or die (mysql_error());
 ...
 //sacar estales
 $sac_aportes_est = mysql_query("select *, mid(s08c_origen,3,4) as org from poa02_origen where mid(s08c_origen,3,4) between 1000 and 1999 and id_poa02= ".$row['obra'],$siplan_data_conn) or die (mysql_error());
	$res_aporte_est= mysql_fetch_array($sac_aportes_est);

	 $sac_aportes_fed = mysql_query("select *, mid(s08c_origen,3,4) as org from poa02_origen where mid(s08c_origen,3,4) between 2000 and 2999 and id_poa02= ".$row['obra'],$siplan_data_conn) or die (mysql_error());
	$res_aporte_fed= mysql_fetch_array($sac_aportes_fed);


	 $sac_aportes_mun = mysql_query("select *, mid(s08c_origen,3,4) as org from poa02_origen where mid(s08c_origen,3,4) between 3000 and 3999 and id_poa02= ".$row['obra'],$siplan_data_conn) or die (mysql_error());
	$res_aporte_mun= mysql_fetch_array($sac_aportes_mun);


	$sac_aportes_otr = mysql_query("select *, mid(s08c_origen,3,4) as org from poa02_origen where mid(s08c_origen,3,4) between 4000 and 4999 and id_poa02= ".$row['obra'],$siplan_data_conn) or die (mysql_error());
	...
	$federal= number_format($res_aporte_fed['monto'],2);

$estatal=number_format($res_aporte_est[',monto'],2);
$municipal=number_format($res_aporte_mun['monto'],2);
$particip=0;//number_format($res_aporte['participantes'],2);
$credito=0;//number_format($res_aporte['credito'],2);
$otros=number_format($res_aporte_otr['monto'],2);
 //sacar aportes
...
$st_edo="Cancelada";
 ...
 <tr class=xl144 height=17 style='height:12.75pt'>
  ...
  <td class=xl115 width=70 style='border-left:none;width:53pt'><?php echo utf8_decode($region); ?></td>
  ...
  <td class=xl115 width=79 style='border-left:none;width:59pt'><?php echo utf8_decode($poa); ?></td>
  <td class=xl115 width=100 style='border-left:none;width:75pt'><?php echo utf8_decode($subpoa); ?></td>
  <td class=xl115 width=134 style='border-left:none;width:101pt'><?php echo utf8_decode($nom_obras); ?></td>
  <td class=xl140 width=60 style='border-left:none;width:45pt'><?php echo $cve_mun; ?></td>
  <td class=xl115 width=100 style='border-left:none;width:75pt'><?php echo utf8_decode($nom_mun); ?></td>
  <td class=xl140 width=60 style='border-left:none;width:45pt'><?php echo $cve_loc; ?></td>
  <td class=xl115 width=100 style='border-left:none;width:75pt'><?php echo utf8_decode($nom_loc); ?></td>
  ...
  <td class=xl115 width=206 style='border-left:none;width:65pt'><?php echo utf8_decode($medida); ?></td>
  ...
  <td class=xl115 width=174 style='border-left:none;width:131pt'><?php echo (utf8_decode($obs)); ?></td>
 </tr>
