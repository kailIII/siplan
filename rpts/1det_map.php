...
FROM regiones INNER JOIN (municipios INNER JOIN (dependencias INNER JOIN (obras INNER JOIN aportes ON obras.id_obra = aportes.id_obra) ON dependencias.id_dependencia = obras.id_dependencia) ON municipios.id_finanzas = obras.municipio) ON regiones.id_region = municipios.id_region and obras.status_obra>=3
...
$mun=mysql_query("select * from municipios where id_finanzas=".$mp,$siplan_data_conn);
...
$chart->render("tmp/ma.png");
 ...
 if($mp=="59") {
	 $regg=mysql_query("select * from regiones",$siplan_data_conn);
//	$row=mysql_fetch_array($mun);
	 $rre=1;
